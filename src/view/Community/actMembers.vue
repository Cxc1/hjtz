<template>
  <div>
    <Header :title="$t('header.lang_list_3')" gobackShowBol="true">
      <router-link tag="span" to="/actRecord" class="slot">{{$t('header.lang_list_4')}}</router-link>
    </Header>
    <div class="main">
      <div class="ipt_box">
        <p>{{$t('content_text.lang_list_5')}}</p>
        <div class="ipt_box_item">
          <input type="text" @blur="mblur" maxlength="30" v-model="userName" :placeholder="$t('content_text.lang_list_6')">
        </div>
        <p>{{$t('content_text.lang_list_8')}}</p>
        <div class="ipt_box_item">
          <input type="text" @blur="mblur" maxlength="11" v-model="userId" :placeholder="$t('content_text.lang_list_7')">
        </div>
          ...
          <div class="menber_box_log_top_r">
            ...
            <div class="menber_box_log_top_r_b">
              ...
              <div class="menber_box_log_top_r_b_r">
                <p @click="activationUser">{{$t('btnTxt.lang_list_11')}}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    methods: {
      ...
      getUserInfo() {
        if (this.userName !== '' && this.userId !== '') {
          ...
          this.$post('/Money/getActivationUserInfo', data).then(res => {
            if (res.code == 10000) {
          ...
          this.resultTxt = this.$t('msgs.lang_list_15')
        } else if (this.userId == '') {
          this.resultTxt = this.$t('msgs.lang_list_16')
        }
      },
      //審核激活社員
      activationUser() {
        this.$loading.open();
        this.$post('/Money/checkActivationUser', {user_id: this.userData.user_id}).then(res => {
          this.$loading.close();
          if (res.code == 10000) {
            Toast(res.message);
            this.resultBol = false;
